...
const (
	TunnelService_Stream_FullMethodName = "/tunnel.TunnelService/Stream"
)
...
type TunnelServiceClient interface {
	...
	Stream(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[EdgeMessage, RelayMessage], error)
}
...
func (c *tunnelServiceClient) Stream(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[EdgeMessage, RelayMessage], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &TunnelService_ServiceDesc.Streams[0], TunnelService_Stream_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[EdgeMessage, RelayMessage]{ClientStream: stream}
	return x, nil
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type TunnelService_StreamClient = grpc.BidiStreamingClient[EdgeMessage, RelayMessage]
...
type TunnelServiceServer interface {
	// Stream establishes persistent bidirectional tunnel
	// Edge initiates connection, relay sends commands, edge streams data back
	Stream(grpc.BidiStreamingServer[EdgeMessage, RelayMessage]) error
...
func (UnimplementedTunnelServiceServer) Stream(grpc.BidiStreamingServer[EdgeMessage, RelayMessage]) error {
...
func _TunnelService_Stream_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(TunnelServiceServer).Stream(&grpc.GenericServerStream[EdgeMessage, RelayMessage]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type TunnelService_StreamServer = grpc.BidiStreamingServer[EdgeMessage, RelayMessage]
